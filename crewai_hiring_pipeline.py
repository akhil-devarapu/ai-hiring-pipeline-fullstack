...
# CrewAI Tasks
def create_resume_screening_task(candidate_data):
    """Task for screening candidate resume"""
    return Task(
        description=f"""
        Analyze the following candidate information and determine if they should proceed to the coding test:
        ...
        """,
        agent=create_resume_screening_agent(),
        expected_output="A JSON response with 'decision' (YES/NO), 'reasoning' (detailed explanation), and 'confidence_score' (0-100)"
    )

def create_coding_assessment_task(candidate_data, question=None, code=None):
    """Task for coding assessment"""
...
def create_technical_interview_task(candidate_data, question=None, answer=None):
...
def create_hr_interview_task(candidate_data, question=None, answer=None):
    """Task for HR interview"""
    if question and answer:
        # Evaluation mode
        return Task(
            description=f"""
...
def create_offer_letter_task(candidate_data):
    """Task for generating offer letter"""
    return Task(
        description=f"""
...
def candidate_form():
    if request.method == 'POST':
        ...
        # Use CrewAI for resume screening
        candidate_data = {
            'name': name,
            'email': email,
            'skills': skills,
        ...
        # Create and run the resume screening crew
        screening_task = create_resume_screening_task(candidate_data)
        screening_crew = Crew(
            agents=[create_resume_screening_agent()],
        ...
        try:
            import re
            decision_match = re.search(r'"decision":\s*"(YES|NO)"', result)
            decision = decision_match.group(1) if decision_match else "NO"
        except:
            decision = "NO" if "NO" in result.upper() else "YES"
...
def coding_test(token):
    ...
    if not question:
        ...
        question_task = create_coding_assessment_task(candidate_data)
        question_crew = Crew(
    ...
    if request.method == 'POST':
        ...
        evaluation_task = create_coding_assessment_task(candidate_data, question, code)
...
def tech_interview(token):
    ...
    if not question:
        ...
        question_task = create_technical_interview_task(candidate_data)
        question_crew = Crew(
    ...
    if request.method == 'POST':
        ...
        evaluation_task = create_technical_interview_task(candidate_data, question, answer)
        evaluation_crew = Crew(
            agents=[create_technical_interview_agent()],
            tasks=[evaluation_task],
            verbose=True,
...
def hr_interview(token):
    ...
    if not question:
        ...
        question_task = create_hr_interview_task(candidate_data)
        question_crew = Crew(
    ...
    if request.method == 'POST':
        ...
        evaluation_task = create_hr_interview_task(candidate_data, question, answer)
        evaluation_crew = Crew(
            agents=[create_hr_interview_agent()],
            tasks=[evaluation_task],
            verbose=True,
        ...
        if analysis_result['recommendation'] == 'PASS':
            ...
            offer_task = create_offer_letter_task(candidate_data)
            offer_crew = Crew(
